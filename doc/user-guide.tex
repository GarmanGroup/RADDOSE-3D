...
Beta version 1.3 -- 19 February 2015\\[3.5em]
\end{center}
...
\end{figure}
\newline
Note that when using the \Keyword{ABSCOEFCALC SAXS} option (section \hyperref[abscoefcalc]{\Keyword{ABSCOEFCALC}}) angle \textit{P} is automatically rotated by an additional 90$^{\circ}$. This is particularly useful for SAXS experiments where the solution is contained in a cylindrical shaped capillary so that the beam direction is perpendicular to the height (axial) dimension of the cylinder.
