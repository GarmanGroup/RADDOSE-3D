...
class NormalEnergyDistribution {
    ...
    // Method to compute parameters
     public void computeValues() {
        ...
        deltaEOverE = energyFwhm / meanEnergy; // need to check that this is how delta E /E is defined in Meents 2017
     }
    ...
    public void   setSigma(double sigma) {sigmaE = sigma;} //sets FWHM
    ...
    public boolean testIfNeglect(double value) {
        double cutAt = 40*sigmaE; //need to check if this is suitable value
...
class SampleNormalEnergyDistribution {
    ...
    SampleNormalEnergyDistribution(double mean, double fwhm, long numberOfPhotonsToSample) {    // pulse energy used in computation of number of photons
    ...
    while(lengthAfterNeglections != targetLength) {
        ...
        for(int i = 0; i < arrayReg.length; i++) {
            double value = (double)(i)/(double)(lengthBeforeNeglections-1); 
            arrayReg[i] = value;
            }
        ...
        lengthBeforeNeglections -= 1; // as added in one extra in final iteration
        
//        System.out.println("\n\nLength before neglections: " + lengthBeforeNeglections + "\nLength after neglections: " + lengthAfterNeglections);
//        System.out.println("The value of sampleSize:" + sampleSize + "\n\n\n");
...
/*    
    ...
    this.sample = normalPhotonEnergiesShuffled;      */ this.sample = normalPhotonEnergies; // no need to shuffle
}
